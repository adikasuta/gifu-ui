    ...
    <BasicForm>
      ...
      <template v-slot:body>
        <ValidationObserver ref="observer">
          ...
          <v-row>
            <v-col cols="4">
              <span>{{ $t("views.administration.fields.email") }}</span>
            </v-col>
            <v-col cols="8">
              <ValidationProvider
                v-slot="{ errors }"
                :name="$t('views.administration.fields.email')"
                vid="email"
                :rules="{
                  email: true,
                  required: true,
              ...
              >
                <v-text-field
                  ...
                  :error-messages="errors"
                  :label="$t('views.administration.fields.email')"
                ></v-text-field>
              </ValidationProvider>
            </v-col>
          </v-row>
          ...
          <v-row>
            <v-col cols="4">
              <span>{{ $t("views.administration.fields.role") }}</span>
            </v-col>
            <v-col cols="8">
              <ValidationProvider
                v-slot="{ errors }"
                :name="$t('views.administration.fields.role')"
                vid="role"
                rules="required"
                ref="provider"
              >
...
import { useAlertMessage } from "../../store/alert-message";
...
export default {
  ...
  computed: {
    ...mapState(useReferenceData, ["genders", "roles"]),
    isUpdate() {
      return !!this.userId;
    },
    isPreview() {
  ...
  methods: {
    ...
    ...mapActions(useAlertMessage, ["pushAlert"]),
      ...
      this.$refs.observer.validate().then(async (success) => {
        if (success) {
          try {
            ...
            const result = await UserService.saveUser(formData);
            if (!this.isUpdate) {
              this.pushAlert(`Generated password=${result.generatedPassword}`);
    ...
    async handleRefresh() {
      try {
        this.isLoading = true;
        this.profile = await UserService.getUser(this.userId);
        this.isLoading = false;
      } catch (error) {
        this.isLoading = false;
        this.pushError(error);
